...
public class Validator {
	...
	public void add(String key, String value, Rule[] ruleArr){
		nameMap.put(key, key);
	...
	public void add(String key, String value, String name, Rule[] ruleArr){
		try {
			nameMap.put(key, Message.get(name));
		} catch (Exception e) {
			nameMap.put(key, key);
		}
		valueMap.put(key, value);
	...
	public void addCn(String key, String value, String cn_name, Rule[] ruleArr){
		nameMap.put(key, cn_name);
		valueMap.put(key, value);
		ruleMap.put(key, ruleArr);
	}
	...
	public String validateResult() throws Exception{
//		String message = null;
		Map<String, Object> resultMap = null;
	...
	public Map<String, Object> validate(String key) throws Exception{
		...
		if(rules!=null && rules.length>0){
			for(Rule rule : rules){
				...
				if(!rule.valid()){
					resultMap.put("result", false);
					resultMap.put("message", nameMap.get(key)==null?key:nameMap.get(key) + rule.getMessage());
					message = nameMap.get(key)==null?key:nameMap.get(key) + rule.getMessage();
					break;
